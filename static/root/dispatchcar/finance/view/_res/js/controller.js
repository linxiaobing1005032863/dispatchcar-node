var app = angular.module('finView', ['toastr']);
app.controller('finViewCtrl', function($scope,finSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    finSer.viewFin(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.dispatchcar.finance.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.dispatchcar.finance.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            ...
            iframe.src=`/finance/download${encode(obj,true)}`;
