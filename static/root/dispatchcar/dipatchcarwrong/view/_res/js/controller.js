var app = angular.module('wrongView', ['toastr']);
app.controller('wrongViewCtrl', function($scope,wrongSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    wrongSer.viewWrong(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.dispatchcar.dipatchcarwrong.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.dispatchcar.dipatchcarwrong.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            if($scope.encloSigning[i].checked &&$scope.encloSigning[i].delel){
                var obj = {
            ...
            iframe.src=`/dipatchcarwrong/download${encode(obj,true)}`;
    ...
    $scope.delFn = function(){
       ...
       /*   $http({
                method: 'POST',
                url: '/dipatchcarwrong/deleteFile',
                headers: {
              ...
              $http({
                      method: 'POST',
                      url: '/dipatchcarwrong/deleteFilee',
                      headers: {
                          'Content-Type': undefined
                      },
                      data: fd,
