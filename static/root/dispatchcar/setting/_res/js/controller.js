var app = angular.module('setting', [{
    files:[
        "root/dispatchcar/setting/_res/js/service.js"
    ]
}]);
app.controller('settingCtrl',function ($scope,$state) {

    if ($state.current.url == '/setting') {//默认加载列表
        $state.go('root.dispatchcar.setting.list[12]')
    }
    ...
    $scope.edit = function(){
        if($scope.editId){
            $state.go('root.dispatchcar.setting.edit[12]',{id:$scope.editId,page:$scope.page});
